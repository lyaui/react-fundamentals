...
- 想想使用 `React.createElement` 時，要印出複數元素時 `{ children: [] }`，因此
  JSX 回傳 map 的陣列也是合情合理的。
- `React.createElement()` JSX 也可以作為 type 傳入 e.g.
  `React.createElement(message, {children: 'Hello World'})`
- 使用 React.createElement 和 直接使用函式回傳 JSX 的差別在於前者會產生
  component，後者不會。
...
<img src="https://i.imgur.com/p1PaGQo.png" width="60%"/>

- 而根據 Ex2 又可以改寫成 `<message> Hello World </message>`，然而 Babel 不認得
  這個 tag，必須改成大寫才行得通。
...
- Ex3 為 component 的 props 設定型別 `Component.propTypes`，但因為可能會造成效能
  問題，如果是 run build 的話，型別判定就會被移除。可以直接使用 react team 的
  prop-types library
